...
validate_manifest() {
  ...
  clash_win_arm_path=$(manifest_get clash_verge.win32_arm64.path) || die "manifest 缺少 Clash Verge Windows arm64 路径"
  clash_win_arm_sha=$(manifest_get clash_verge.win32_arm64.sha256) || die "manifest 缺少 Clash Verge Windows arm64 摘要"
  clash_win_x64_path=$(manifest_get clash_verge.win32_x64.path) || die "manifest 缺少 Clash Verge Windows x64 路径"
  clash_win_x64_sha=$(manifest_get clash_verge.win32_x64.sha256) || die "manifest 缺少 Clash Verge Windows x64 摘要"
  clash_win_arm_status=$(manifest_get clash_verge.win32_arm64.signature_status) || die "manifest 缺少 Clash Verge Windows arm64 签名状态"
  ...
  valid_relative_path "$clash_win_x64_path" || die "Clash Verge Windows x64 路径无效"
  ...
  [ "$(printf '%s' "$claude_win_arm_sha" | lowercase)" = "$EXPECTED_CLAUDE_WIN_ARM64_SHA" ] || die "Claude Code Windows arm64 摘要不匹配启动器固定基线"
  [ "$(printf '%s' "$claude_win_x64_sha" | lowercase)" = "$EXPECTED_CLAUDE_WIN_X64_SHA" ] || die "Claude Code Windows x64 摘要不匹配启动器固定基线"
