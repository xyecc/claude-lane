...
- **main（本分支）= Agent 一键主干**，也是本仓库的长期形态：一条命令装好 Clash，装上 Claude Code 后由 agent 读本仓库、向你要凭证、配好链式代理。macOS 已完整实现；**Windows 的 agent 版是下一个大版本（v1.4）的目标**——架构为「agent 当指挥、脚本当工具」。
- **`codex/bootstrap-v1.3.0`（侧分支，冻结保留，不并入 main）**：脚本/检查点实验版——国内 OSS 固定版分发 + Windows 纯脚本部署路径 + 五机验收发布流水线。经一台 Windows 11 x64 真机验收打磨出 9 项真机修复（已装 Clash 检测、注册表引号、TEMP 异常、脚本块作用域、节点自动扫描、API 编码、TUN 探测等），链式分流已在真机实际跑通。这些经过验证的脚本（六项验证 / 备份回滚 / 证据生成）将在 v1.4 里作为 agent 调用的工具箱回归主干。该分支候选构建冻结于 RC `75f63f0a0fce`，**分支上任何文件改动都会使 RC 归档摘要失效**，故保持只读。
