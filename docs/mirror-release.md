...
发布机环境：macOS 13+、系统 Bash 3.2；额外需要 GnuPG。所有脚本默认 dry-run，只有 `--execute` 才产生下载、归档或上传。

## 固定基线

| 产品 | 版本 | 平台 |
|---|---:|---|
| Claude Code | `2.1.212` | darwin-arm64、darwin-x64 |
| Clash Verge Rev | `2.5.2` | macOS arm64 / x64 DMG |
| claude-lane | `1.3.0` | tar.gz |
...
Intel 版 Claude Code 的 `--version` 仍需在原生 Intel Mac 上复验。lane 包只能从干净、已审阅并已提交的工作树构建；归档使用运行时文件白名单，不包含外层 `bootstrap.sh`、`manifests/stable.json` 或发布者工具，从而避免 manifest 与 tarball 摘要互相引用：
...
`generate-manifest.sh` 只生成 `.mirror-work/manifests/stable.candidate.json`，不会把状态改成 released。完成主备回下载、两种 Mac 真机和无代理端到端验收后，由维护者审阅并清空全部 blocker、填写两个受控 HTTPS 网关，再显式运行：
...
晋级脚本要求清单已是 `released`、blocker 为空、主备网关均为 HTTPS 且 Git 工作树干净。当前仓库不满足这些条件，拒绝晋级是预期结果。
