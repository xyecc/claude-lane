...
查自己是哪一版：`cat VERSION`，或跑 `bash scripts/verify.sh` 看末尾的 `[版本]` 行（它会顺便告诉你本机部署的是不是这一版）。
...
- FAQ 与购买渠道拆到 `docs/faq.md`；「分支与路线图」移到本文件末尾
...
- **main = Agent 一键主干**，也是本仓库的长期形态：一条命令装好 Clash，装上 Claude Code 后由 agent 读本仓库、向你要凭证、配好链式代理。macOS 已完整实现；**Windows 的 agent 版是下一个大版本（v1.4）的目标**——架构为「agent 当指挥、脚本当工具」。
