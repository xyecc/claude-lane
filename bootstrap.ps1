...
$ExpectedClaudeVersion = "2.1.212"
$ExpectedClashVersion = "2.5.2"
$Expected = @{
    "claude-win32-arm64" = "adaa6e3dadb8016755ccd1907a5f249c1bc9bdb6c71d3f7dcea7d5db8f72d0a5"
    "claude-win32-x64" = "fe639693fd7e9a881c799867711abb7666dec2a5fefbaba41af6a09e71bcbefa"
...
# Release block: promotion writes fixed HTTPS gateway URLs and this immutable digest.
$PrimaryBaseUrl = "https://TBD-PRIMARY.invalid"
$BackupBaseUrl = "https://TBD-BACKUP.invalid"
...
function Get-CheckedArtifact([string]$Path, [string]$Sha256, [string]$Destination, [string]$Label) {
    foreach ($Base in @($PrimaryBaseUrl, $BackupBaseUrl)) {
        ...
        try {
            ...
            Write-Warning "$Label 摘要不一致，尝试备用国内源"
        } catch {
            Write-Warning "$Label 下载失败，尝试备用国内源"
        }
    }
    Stop-Bootstrap "$Label 的主备源均未通过下载与 SHA-256 校验"
}
...
try {
    ...
    if ([string]::IsNullOrWhiteSpace($ManifestFile)) {
        foreach ($Base in @($PrimaryBaseUrl, $BackupBaseUrl)) {
            if (Test-Placeholder $Base -or -not $Base.StartsWith("https://", [StringComparison]::OrdinalIgnoreCase)) { Stop-Bootstrap "国内主备下载源尚未发布，启动器保持失败关闭" }
        }
    ...
    foreach ($EntryPath in @($LanePath, [string]$ClaudeEntry.path, [string]$ClashEntry.path)) { if (-not (Test-RelativePath $EntryPath)) { Stop-Bootstrap "manifest 含无效 Windows 路径" } }
    ...
    if ([string]$ClaudeEntry.signature_status -ne "verified-authenticode" -or [string]$ClashEntry.signature_status -ne "verified-authenticode") { Stop-Bootstrap "真实 Windows Authenticode 证据尚未完成" }
    ...
    Get-CheckedArtifact ([string]$ClaudeEntry.path) ([string]$ClaudeEntry.sha256) $ClaudeDownload "Claude Code"
    ...
    Assert-Authenticode $ClaudeDownload 'O=Anthropic, PBC' "Claude Code"
    Assert-Authenticode $ClashDownload 'Clash|Verge' "Clash Verge Rev"
    ...
    if (Test-Path -LiteralPath $ClaudeTarget) {
        ...
        Assert-Authenticode $ClaudeTarget 'O=Anthropic, PBC' "现有 Claude Code"
    } else {
    ...
    if ($KnownClash.Count -eq 0) {
        Write-Output "启动已验签的 Clash Verge Rev 安装器；请在 Windows 界面中确认安装。"
    ...
    Write-Output "Windows 固定版下载与安装阶段完成；专线路由仍须按 docs/porting.md 人工执行。"
} finally {
